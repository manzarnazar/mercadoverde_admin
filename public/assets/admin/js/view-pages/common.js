...
(function () {
    ...
    const zoneForm = document.getElementById("zone_form");
    if (!mapCanvas || !zoneForm) {
        return;
    }

    function injectZoneMapStyles() {
        if (document.getElementById("zone-map-v2-styles")) {
            return;
        }
        const style = document.createElement("style");
        style.id = "zone-map-v2-styles";
        style.textContent = [
            ".zone-map-drawing-controls{display:flex;gap:4px;margin:10px 0 4px;background:#fff;border-radius:4px;box-shadow:0 2px 6px rgba(0,0,0,.3);padding:4px}",
            ".zone-map-tool-btn{width:36px;height:36px;border:0;border-radius:4px;background:#fff;color:#333;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:18px}",
            ".zone-map-tool-btn:hover,.zone-map-tool-btn.active{background:#0177cd;color:#fff}",
            ".zone-map-finish-wrap{margin-bottom:6px}",
            ".zone-map-finish-btn{display:none;border:0;border-radius:4px;background:#0177cd;color:#fff;padding:6px 14px;font-size:12px;cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.3)}",
            "#map-canvas .zone-map-drawing-controls,#map-canvas .zone-map-finish-wrap{transform:translateY(10px)}",
            ".gm-style #pac-input{display:block!important;height:40px;border:1px solid #fbc1c1;outline:none;box-shadow:none;top:7px!important;left:7px!important;padding-inline-start:10px;min-width:180px}"
        ].join("");
        document.head.appendChild(style);
    }

    if (!window.ZoneMapDrawing) {
        ...
        ZoneMapDrawingInstance.prototype._createDrawingControls = function () {
            ...
            this.panButton = this._makeControlButton("Hand tool", "tio-hand-draw", () => this.setPanMode());
            this.drawButton = this._makeControlButton("Shape tool", "tio-free-transform", () => this.setDrawMode());
            wrapper.appendChild(this.panButton);
            wrapper.appendChild(this.drawButton);
            wrapper.appendChild(this._makeControlButton("Reset polygon", "tio-clear", () => this.clearPolygon()));
        ...
        ZoneMapDrawingInstance.prototype._setActiveTool = function (activeButton) {
            [this.panButton, this.drawButton].forEach((button) => button.classList.remove("active"));
            if (activeButton) {
        ...
        ZoneMapDrawingInstance.prototype.setDrawMode = function () {
            ...
            this._removeDrawingListeners();
            this.mapClickListener = this.map.addListener("click", (event) => this._addVertex(event.latLng));
        ...
        ZoneMapDrawingInstance.prototype.finishDrawing = function () {
            ...
            const path = this.drawingPath.map((latLng) => ({ lat: latLng.lat(), lng: latLng.lng() }));
            this._clearDrawingPreview();
    ...
    function readInitialPaths() {
        if (mapCanvas.dataset.zonePaths) {
            try {
                return JSON.parse(mapCanvas.dataset.zonePaths);
            } catch (error) {
                console.warn("Invalid zone paths data attribute", error);
            }
        }
        return null;
    }

    function bootstrapZoneMap() {
        injectZoneMapStyles();

        const defaultCenter = {
            lat: parseFloat(mapCanvas.dataset.zoneLat) || 23.757989,
            lng: parseFloat(mapCanvas.dataset.zoneLng) || 90.360587,
        };

        const initialPaths = readInitialPaths();
        const options = {
        ...
        if (initialPaths && initialPaths.length) {
            options.initialPaths = initialPaths;
            options.initialPolygonOptions = { fillOpacity: 0 };
        }

        window.zoneMapInstance = ZoneMapDrawing.init(options);
        window.map = window.zoneMapInstance.map;

        Object.defineProperty(window, "lastpolygon", {
            get: function () {
                return window.zoneMapInstance.lastPolygon;
            },
            set: function (value) {
                window.zoneMapInstance.lastPolygon = value;
            },
        });

        if (typeof window.set_all_zones === "function") {
            window.set_all_zones();
        }
    }
    ...
    window.__zoneMapV2 = true;

    if (window.google && window.google.maps && !document.querySelector(".zone-map-drawing-controls")) {
        bootstrapZoneMap();
    }
})();
...
(function () {
    ...
    if (searchInput) {
        searchInput.addEventListener("search", function () {
            if (!this.value.trim() && window.jQuery) {
                window.jQuery("#searchResults").html('<div class="text-center text-muted py-5"></div>');
            }
        });
    }
})();
