@php
    ...
    $showAllFields = $showAllFields ?? false;
    $hasIdentityDocs = $store->tin_certificate_image
        ...
        || $store->tin;
@endphp
@if ($hasIdentityDocs || $showAllFields)
                        ...
                        @foreach ($identityDocuments as $document)
                            ...
                            @if ($fileName || $showAllFields)
                                <div class="col-md-6 col-xl-3">
                                    ...
                                    @else
                                        <div class="bg--secondary rounded p-20 text-center">
                                            <img width="40" height="40"
                                                src="{{ asset('public/assets/admin/img/upload-cloud.png') }}" alt="">
                                            <p class="fs-12 mb-0 mt-2 text-muted">{{ translate('messages.not_found') }}</p>
                                        </div>
                                    @endif
                                </div>
                            @endif
